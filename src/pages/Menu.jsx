...
export default function Menu() {
  ...
  const [cart, setCart] = useState(
    ...
    menuItems.reduce((obj, item) => ({ ...obj, [item.id]: 0 }), {})
  );
  ...
  const total = menuItems.reduce(
    (sum, item) => sum + cart[item.id] * item.price,
    0
  );
  ...
  const handleCheckOut = () => {
    setTimeout(() => {
      const Order = menuItems.reduce((obj, item) => {
        if (cart[item.id] > 0) {
          return {
            ...obj,
            [item.id]: {
      ...
      localStorage.setItem(`${username}-order`, JSON.stringify(Order));
              ...
              if (cart[itemId] > 0) {
                return (
                  <li key={item.id} className="cart-item">
                    <span>
                      {item.name}: {cart[itemId]} x ${item.price} = $
                      {cart[itemId] * item.price}
                    </span>
                    <button
                      className="decrement-btn"
                      onClick={() => adjustQuantity(item.id, -1)}
